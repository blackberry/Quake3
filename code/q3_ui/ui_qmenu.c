...
sfxHandle_t Menu_DefaultKey( menuframework_s *m, int key )
{
	...
	if (item && !(item->flags & (QMF_GRAYED|QMF_INACTIVE)))
	{
		switch (item->type)
		{
			...
			case MTYPE_FIELD:
				...
				if (key != K_ENTER && key != K_TAB) // Make sure to popup keyboard only when a text field is clicked.
					showKeyboard = true;
				break;
		}
	...
	switch ( key )
	{
		...
		case K_MOUSE3:
			if (item)
				...
				{
					// If VKB is required for this field then show it.
					if (showKeyboard)
						VKB_ShowKeyboard();
	...
	// If VKB is not required for a given field then hide it.
	if (!showKeyboard)
		VKB_HideKeyboard();
