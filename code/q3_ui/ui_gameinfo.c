...
void UI_LogAwardData( int award, int data ) {
	...
	if( award > AWARD_PERFECT ) {
		UI_trap_Print( va( S_COLOR_RED "Bad award %i in UI_LogAwardData\n", award ) );
		return;
	}
...
void UI_InitGameinfo( void ) {
	...
	if( (UI_trap_Cvar_VariableValue( "fs_restrict" )) || (ui_numSpecialSinglePlayerArenas == 0 && ui_numSinglePlayerArenas == 4) ) {
