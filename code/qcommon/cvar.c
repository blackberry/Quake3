...
============
*/
static cvar_t *Cvar_FindVar( const char *var_name, long hash ) {
	cvar_t	*var;

	if (hash < 0)
		hash = generateHashValue(var_name);
	assert(hash < FILE_HASH_SIZE && hash >= 0);
...
cvar_t *Cvar_Get( const char *var_name, const char *var_value, int flags ) {
  ...
  if ( !var_name || ! var_value ) {
	...
	hash = generateHashValue(var_name);
	var = Cvar_FindVar (var_name, hash);
	if ( var ) {
