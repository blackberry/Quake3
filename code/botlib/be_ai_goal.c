...
// RIM_NDK WARNING!!!! g_gametype has been changed to g_gametype_int in all 
//  cases within this file in order to fix confusion with global variable
//  g_gametype found in file g_local.h
...
//the game type
int g_gametype_int = 0; // bk001206 - init
...
int BotGetLevelItemGoal(int index, char *name, bot_goal_t *goal)
{
	...
	{
		//
		if (g_gametype_int == GT_SINGLE_PLAYER) {
			if (li->flags & IFL_NOTSINGLE) continue;
		}
		else if (g_gametype_int >= GT_TEAM) {
...
int BotSetupGoalAI(void)
{
	...
	g_gametype_int = LibVarValue("g_gametype", "0");
