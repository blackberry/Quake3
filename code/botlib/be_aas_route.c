...
void AAS_CalculateAreaTravelTimes(void)
{
	...
	for (i = 0; i < aasworld.numareas; i++)
	{
		...
		settings = &aasworld.areasettings[i];
		//
		size += settings->numreachableareas * sizeof(unsigned short *) * 2;
		//
		size += settings->numreachableareas * revreach->numlinks * sizeof(unsigned short);
	} //end for
	//allocate memory for the area travel times
	ptr = (char *) GetClearedMemory(size);
	aasworld.areatraveltimes = (unsigned short ***) ALIGN(ptr);
	ptr = (char*)aasworld.areatraveltimes + aasworld.numareas * sizeof(unsigned short **);
	...
	for (i = 0; i < aasworld.numareas; i++)
	{
		...
		settings = &aasworld.areasettings[i];
		//
		aasworld.areatraveltimes[i] = (unsigned short **) ALIGN(ptr);
		ptr = (char*)aasworld.areatraveltimes[i] + settings->numreachableareas * sizeof(unsigned short *);
		//
		for (l = 0; l < settings->numreachableareas; l++)
		{
			aasworld.areatraveltimes[i][l] = (unsigned short *) ALIGN(ptr);
			ptr = (char*)aasworld.areatraveltimes[i][l] + revreach->numlinks * sizeof(unsigned short);
			//reachability link
