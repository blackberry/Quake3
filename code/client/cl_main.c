...
void CL_ServerStatusResponse( netadr_t from, qmsg_t *msg );
...
====================
*/
void CL_WriteDemoMessage ( qmsg_t *msg, int headerBytes ) {
	int		len, swlen;
...
void CL_NextDemo( void ) {
    ...
    sprintf(cmd_rundemo, "demo %s", v, sizeof(cmd_rundemo));
...
void putenv(const char* env)
{
    ...
    while (env[l] != '=' && env[l] != '\0') l++;

    if (env[l] == '\0' || l == 0)
    {
        /* no '=' in this string. or no key */
        return;
    }
    ...
    while (*s != '\0')
    {
            ...
            s[l] == '=')
        {
            ...
            memcpy(s, t, 2048 - (t - envstring));
			memset(envstring + 2048 - n, 0, n);
...
===================
*/
void CL_ServersResponsePacket( netadr_t from, qmsg_t *msg ) {
...
=================
*/
void CL_ConnectionlessPacket( netadr_t from, qmsg_t *msg ) {
	char	*s;
	char	*c;
...
===================
*/
void CL_ServerInfoPacket( netadr_t from, qmsg_t *msg ) {
	int		i, type;
	char	info[MAX_INFO_STRING];
	char*	str;
	char	*infoString;
